...
if __name__ == '__main__':
    ...
    data = pd.read_csv('../data/thread_sw2000.csv', encoding='utf-8')
    text = []
    for doc in data['space_sw']:
        d = doc.split()
        text.append(d)

    for topic_num in range(6,7):
        topic = BitermModel(text, ntopics=topic_num, niter=500)
        doc_probs = topic.infer_documents()
        result = pd.DataFrame()
        result['thread_id'] = data['thread_id']
        result['topic'] = doc_probs.values()
        result.to_csv('../data/thread_biterm_topics_{}.csv'.format(topic_num), encoding='utf-8-sig', index=False)

        f1 = open('../data/thread_biterm_words_{}.txt'.format(topic_num), 'w', encoding='utf-8-sig')
        words = topic.get_topic_words(n=30)
